...
const schema11 = { $id: 'Primitive_RegEx', default: 'foo', type: 'string', pattern: 'foo' }
  ...
  if (errors === 0) {
    if (typeof data === 'string') {
      if (!pattern0.test(data)) {
        validate10.errors = [{ instancePath, schemaPath: '#/pattern', keyword: 'pattern', params: { pattern: 'foo' }, message: 'must match pattern "' + 'foo' + '"' }]
        return false
      }
    } else {
